...
/** 解析红包消息（支持带前缀文字） */
export function parseRedPacketInline(content: string): { prefix: string; redPacketId: string } | null {
  const match = RED_PACKET_TAG_REGEX.exec(content);
  if (!match) return null;
  const prefix = content.replace(match[0], '').trim();
  return { prefix, redPacketId: match[1] };
}
